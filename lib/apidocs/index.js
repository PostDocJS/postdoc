...
import {generateCliApiPages} from './from-cli.js';
import {
  info,
  LineBuilder,
  MessageBuilder,
...
import { compileEjs } from '../bundler/page/compile-ejs.js';
 ...
 */
const createCommentParserFor = (file, configuration) => (comment) =>
...
const getAPIExtractor = pipe(
  ({apiExtractor}) => {
    ...
    const extractorOptions = maybeExtractorWithOptions
      ? apiExtractor[extractorName] || {}
      : {};

    return [extractorName, extractorOptions];
  },
  Future.of,
  Future.map(([extractorName, extractorOptions]) =>
    Future.merge(extractorName, extractorOptions, import(extractorName))
  ),
  Future.map(
    ([_extractorName, extractorOptions, {parseComments}]) =>
      ([file, code]) => {
        try {
          return [
            file,
            parseComments(code, {...extractorOptions, skipSingleStar: true})
          ];
        } catch (error) {
          MessageBuilder()
            .line(
 ...
 */
const getTemplateFile = pipe(resolve, File, (file) => file.content());
 ...
 */
const createApiDocFile = (templateContent) =>
  pipe(
    (apiFile) => Future.merge(apiFile, templateContent),
    Future.map(async ([apiFile, templateContent]) => ({
      file: apiFile.file,
      content: await compileEjs(templateContent, apiFile.entity, {filename: apiFile.file})
    }))
  );
 ...
 */
const createOutputPathResolver = (
  configuration,
  outputDirectoryName,
  sourceDirectory
) =>
  pipe(
    ({file, content}) => ({
      file: file.replace(File(file).extension(), EJS_EXTENSION),
      content
    }),
    ({file, content}) => ({
      file: file.replace(`${resolve(sourceDirectory)}${sep}`, ''),
      content
    }),
    ({file, content}) => ({
      file: resolve(configuration.directories.pages, outputDirectoryName, file),
      content
    })
  );
...
export const generateApiDocFiles = (
  ...
  if (fromCli) {
    if (outputDirectoryName === configuration.apidocs.outputDirectory) {
      logError('--from-cli option requires the --output <path-to-package-dir> option.');
    
      exit(1);
    }

    return Future.of(
      generateCliApiPages(configuration, resolve(outputDirectoryName), stdout)
    );
  }
  ...
  return pipeWith(
    ...
    Iterable.filter(not(isPartial)),
    Iterable.map((file) => Future.merge(file, file.content())),
    Iterable.map(Future.apply(getAPIExtractor(configuration))),
    Future.merge,
    Future.map(
      pipe(
        Iterable.from,
        Iterable.map(([file, comments]) => [
          file,
        ...
        Iterable.filter(([_file, comments]) => comments.length),
        Iterable.map(([file, comments]) => [
          file.source(),
          comments.map(createCommentParserFor(file, configuration))
        ]),
        Iterable.chain(([file, entities]) =>
          entities.map((entity) => ({file, entity}))
        ),
        Iterable.map(createApiDocFile(getTemplateFile(templatePath)))
      )
    ),
    Future.map((data) => {
      debugger;

      return stdout ? logDataToStdout(data) : data;
    }),
    Future.map(
      Optional.map(
        pipe(
    ...
    Optional.transpose(Future.of)
  );
};

const logDataToStdout = pipe(Array.from, (data) =>
  MessageBuilder().line(inspect(data)).pipe(info)
);
