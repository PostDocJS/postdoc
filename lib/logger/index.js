...
import {Typography} from './colors.js';
...
/** @type {Logger} */
export const info = log(LoggerLevel.INFO);
/** @type {Logger} */
export const warn = log(LoggerLevel.WARNING);
/** @type {Logger} */
export const error = log(LoggerLevel.ERROR);
...
export const LineBuilder = () => {
  ...
  const API = {
    /**
     * Defines a mapper which will modify
     * the final message. There is ability to
     * define multiple mappers. In that case,
     * the order of defining mappers describing
     ...
     * The position of the invocation matters.
     *
     * @param {number} count
     * @param {string} value
     */
     ...
     * @returns {string}
     */
    build: () => pipe(...transformers)(prefix + message + suffix)
  };
...
export const MessageBuilder = () => {
  ...
  const API = {
     ...
     * invocation order of the method.
     *
     * @param {string} text
     */
    line: (text) => (lines.push(text), API),
    /**
     * Defines a line of the message when *predicate*
     * returns `true`.
     * The position in the message is defined by the
     * invocation order of the method.
     *
     * @param {LoggerIfPredicate} predicate
     * @param {TextGetter} getText
     */
    lineIf: (predicate, getText) => {
      if (predicate()) {
        lines.push(getText());
      }
     ...
     */
    pipe: (logger) => {
      const mapLine = pipe(...transformers);

      lines.forEach(pipe(mapLine, logger));
    },
    /**
     * Pass lines to a _logger_ instance when the _predicate_
     * parameter returns `true`. Otherwise, it passes the lines
     * to the _elseLogger_ if it is defined.
     * May be useful instead of the _build_ method.
     *
     * @param {LoggerIfPredicate} predicate
     * @param {Logger} logger
     * @param {Logger} [elseLogger]
     * @returns {void}
     */
    pipeIf: (predicate, logger, elseLogger) => {
      const mapLine = pipe(...transformers);
      const finalLogger = predicate() ? logger : elseLogger;

      if (finalLogger) {
        lines.forEach(pipe(mapLine, finalLogger));
      }
    },
    /**
     * Joins all lines.
     ...
     * @returns {string}
     */
    build: () => pipe(...transformers)(lines.join(Separator.NewLine))
  };
 ...
 * @param {Date} duration
 */
export const StatusLine = (duration) =>
  LineBuilder()
    .text('Date:')
...
/**
 * A pattern for the line which will contain
 * a path to the source file from which log has
 * been happened.
 *
 * @param {import('../files.js').IFile} file
 * @returns {string}
 */
export const LocationLine = (file) =>
  LineBuilder()
