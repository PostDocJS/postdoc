...
export default class Page {
  ...
  constructor(contentFilePath, temporaryOutputDirectoryName) {
  ...
  #parseFrontMatter(frontMatter) {
    frontMatter = parse(frontMatter.trim());
    ...
    if (error) {
      Logger.log(
        (typography) =>
          `
            The front matter of ${typography.grey.bold(
              this.#contentFilePath,
            )} contains an invalid known property:

            ${error.details
              .map(({ message }) => typography.red(message))
              .join("\n")}
          `,
        Logger.ErrorLevel,
      );
  ...
  async #compileContent() {
    ...
    const [, frontMatter, separatedContent] = FRONT_MATTER_AND_CONTENT_RE.exec(
      content,
    ) ?? [null, null, content];
    ...
    try {
      const compiledContent = marked.parse(separatedContent.trim());

      return [parsedFrontMatter, compiledContent];
    } catch (error) {
      Logger.log(
        (typography) =>
          `
  ...
  async compile(pages) {
    const compiledContentAndFrontMatter = await this.#compileContent();
