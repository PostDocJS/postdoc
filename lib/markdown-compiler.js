...
export default class MarkdownCompiler {
  ...
  async initialise() {
    ...
    this.#compiler.use(
      markedHighlight({
        highlight(code, lang) {
          ...
          const firstContentfulLineIndex = lines.findIndex(
            (line) => line.length,
          );
    ...
    configuration.markdown.extensions.forEach((extension) =>
      this.#compiler.use(extension),
    );
  }
  ...
  async compile(content) {
    ...
    return [
      attributes,
      html.replace(INTERPOLATION_RE, (_, variableName) => {
