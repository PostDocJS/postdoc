    ...
    return () => {
      app.use(async (request, response, next) => {
        if (request.url.endsWith(".html") || request.url.endsWith("/")) {
          const page = this.#collector.pages.find(
            (page) =>
              page.url === request.url ||
              page.url === request.url + "index.html",
          );

          if (page) {
            let pageHtml = await page.compile(this.#collector.pages);
...
export default class ViteConfiguration {
  static async createForProduction(temporaryDirectoryPath) {
    ...
    const htmlPages = await pipeWith(
      directoryDescriptor.files,
      AsyncIterable.filter((file) => file.endsWith(".html")),
      AsyncIterable.enumerate,
      AsyncIterable.fold({}, (pages, [file, index]) => {
        pages[index] = file;
  ...
  constructor(appType, collector) {
    ...
    this.plugins = [
      new PostDocVitePlugin(collector),
      VitePWA(configuration.pwa),
    ];
    this.build = {
      outDir: resolve(configuration.directories.output),
      emptyOutDir: true,
    };
  }
}
