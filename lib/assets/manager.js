...
async function generateComponentTemplate(templatePath, outputPath, data) {
  const template = await fs.readFile(templatePath, 'utf8');
...
function createPage(templateName, outputPath) {
  const templatePath = path.join(directories.files, `${templateName}.ejs`);

  return () => generatePageTemplate(templatePath, outputPath);
}

function createComponent(templateName, outputPath, data) {
  const templatePath = path.join(directories.files, `${templateName}.ejs`);

  return () => generateComponentTemplate(templatePath, outputPath, data);
}
...
function createCss(cssTemplateName, outputPath) {
  const cssTemplatePath = path.join(directories.styles, `${cssTemplateName}.css`);
...
function cpoyAsset(fileName, destination) {
  const source = path.join(directories.images, fileName);
