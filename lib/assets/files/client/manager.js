...
const trapAnchors = () =>
  document.body.querySelectorAll('a[href]').forEach((anchor) =>
    anchor.addEventListener('click', (event) => {
      ...
      globalThis[GLOBAL_MANAGER_NAME].navigate(shortenSameSiteURI(anchor.href));
    })
  );

if (!globalThis[GLOBAL_MANAGER_NAME]) {
  /**
  ...
  addEventListener(
    ...
    ({state}) =>
      state?.uri &&
      globalThis[GLOBAL_MANAGER_NAME].navigate(shortenSameSiteURI(state.uri))
  );
