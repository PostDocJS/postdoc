...
import {getShortUrl, getUrl, shortenSameSiteURI} from './utilities.js';
...
if (!(GLOBAL_MANAGER_NAME in globalThis)) {
  ...
  window[GLOBAL_MANAGER_NAME] = {
    ...
    navigate: async (url, replaceContext = false) => {
      const shortURL = shortenSameSiteURI(url);

      // If shortURI is null then it means that user tries to
      ...
      await startTransition(shortURL.pathname);
      ...
      await Promise.all(
        events.get(NavigationEventName.AfterTransition).map(([hook, {forPage, registeredOn}]) => {
          if (
            registeredOn.pathname === shortURL.pathname ||
              forPage instanceof RegExp && forPage.test(shortURL.href) ||
              typeof forPage === 'function' && forPage(shortURL)
          ) {
            return hook(shortURL);
          }
        })
