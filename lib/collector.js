...
export default class Collector {
  ...
  constructor(isVirtualTemporaryOutputDirectory) {
    ...
    const temporaryOutputDirectoryPathPrefix = resolve(this.#configuration.directories.output, '..', '.pd-tmp-');
    ...
    try {
      ...
      if (error instanceof PostDocError) {
        Logger.log((typography) => `
    					The following API file does not have a corresponding layout: ${typography.dim(filePath)}.
    						Please add at least a default one ${typography.bold(resolve(this.#configuration.directories.layouts, this.#configuration.apidocs.layout))} or check if it is accidentally ignored.
    				`, Logger.ErrorLevel);
      } else {
        Logger.log((typography) => `
  ...
  #collectApiDocs() {
    ...
    return pipeWith(this.#configuration.apidocs.source, resolve, walkDirectory, ({ files }) => files, AsyncIterable.filter((filePath) => filePath.endsWith('.js')), AsyncIterable.filter((filePath) => !shouldIgnore(filePath)), AsyncIterable.map((filePath) => this.#tryCreatingPage(filePath, ApiPage)), AsyncIterable.filter(Boolean), AsyncIterable.chain((page) => AsyncIterable.from(async function* () {
  ...
  #collectRegularPages() {
    ...
    AsyncIterable.filter((filePath) => filePath.endsWith('.md')), AsyncIterable.filter((filePath) => !shouldIgnore(filePath)), AsyncIterable.map((filePath) => this.#tryCreatingPage(filePath, RegularPage, markdownCompiler)), AsyncIterable.filter(Boolean), AsyncIterable.chain((page) => AsyncIterable.from(async function* () {
      // We should be sure that the page has at least one comment.
      if (await page.shouldCompile()) {
        yield page;
      }
  ...
  #collectLayoutPages() {
    ...
    return pipeWith(this.#configuration.directories.layouts, resolve, walkDirectory, ({ files }) => files, AsyncIterable.filter((file) => file !== apidocsLayout), AsyncIterable.filter((filePath) => filePath.endsWith('.ejs')), AsyncIterable.filter((filePath) => !shouldIgnore(filePath)), AsyncIterable.map((filePath) => this.#tryCreatingPage(filePath, LayoutPage)), AsyncIterable.filter(Boolean), AsyncIterable.chain((page) => AsyncIterable.from(async function* () {
      // We should be sure that the page has at least one comment.
      if (await page.shouldCompile()) {
        yield page;
      }
  ...
  async collectPages() {
    ...
    const pages = await pipeWith(this.#collectRegularPages(), AsyncIterable.concat(this.#collectApiDocs()), AsyncIterable.fold([], (pages, page) => {
      pages.push(page);
    ...
    await pipeWith(this.#collectLayoutPages(), AsyncIterable.filter((page) => !layoutsUsedInPages.has(page.layoutFilePath)), AsyncIterable.forEach((page) => pages.push(page)));
