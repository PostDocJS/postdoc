...
import {resolveConfigWithDir} from './resolve.js';
 ...
 */
export const initializeConfiguration = async function (dir = process.cwd()) {
  config({debug: Boolean(env.DEBUG)});

  const resolvedConfiguration =
    await resolveConfigWithDir(dir).then(injectEnvironments);

  resolvedConfiguration.configuration.directories = Object.entries(
    resolvedConfiguration.configuration.directories
  ).reduce(
