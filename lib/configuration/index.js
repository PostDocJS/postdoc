...
const {resolveConfiguration} = require('./resolve.js');
...
const initializeConfiguration = async () => {
  ...
  const directories = Object.entries(resolvedConfiguration.directories)
    .reduce(
      (accumulator, [key, value]) =>
        // Resolve and normalize directories, so we won't do that later.
        ({...accumulator, [key]: value ? resolve(normalize(value)) : value}),
      {}
    );

  return {...resolvedConfiguration, directories};
};
