 ...
 */
const isApiFile = (file, configuration) =>
  typeof configuration.directories.api === 'string'
    ? file.source().startsWith(configuration.directories.api)
    : configuration.directories.api.some((path) =>
      file.source().startsWith(path)
    );
...
const rebuild = async ({type, file}) => {
  ...
  if (
    isPage(file) &&
    (type === FSEventType.Add || type === FSEventType.Remove)
  ) {
    MessageBuilder()
      ...
      .line(
        LineBuilder()
          ...
          .phrase(
            `file was ${
              type === FSEventType.Add ? 'added' : 'deleted'
    ...
    getCacheKeyDescriptorsByParts(file.source()).forEach(removeCacheEntry);

    pushToClient({
      type: type === FSEventType.Add ? PageEventType.Add : PageEventType.Remove
    });
  } else {
    invalidatedPages = pages.filter(
      (page) =>
        page.layout.source() === file.source() ||
        page.content.source() === file.source() ||
        getCacheKeyDescriptorsByParts([page.output.source(), file.source()])
          .length > 0
    );
        ...
        MessageBuilder()
          ...
          .line(
            LineBuilder()
              ...
              .phrase(
                `file has been ${
                  type === FSEventType.Add
                    ? 'added'
                    : type === FSEventType.Change
                      ? 'changed'
                      : 'removed'
                }.`
              )
  ...
  await Promise.all(
    invalidatedPages.map(async (page) => {
...
export const serveProject = async (options) => {
  ...
  pipeWith(
    ...
    Stream.map((change) => {
      if (
        ...
        isApiFile(change.file, configuration)
      ) {
        clearCache();
    ...
    Stream.map((change) => {
      if (
        change.type === FSEventType.Add ||
        change.type === FSEventType.Remove
      ) {
  ...
  await startServer(configuration, {
    server: {
      ...
      $$openByCLI: determineBrowserAutoOpening(
