...
const {removeCacheEntry, removePartialFromCaches} = require('./cache.js');
    ...
    const affectedPages = pages
      .filter(
        (page) =>
          ...
          || Array.from(page._cache.keys()).some(
            (path) => withoutExtension(file) === withoutExtension(path)
          ) 
      );
...
const rebuildMarkdown = createRebuildStream(
  ({file}) => file.endsWith(MD_SUFFIX) ? file : undefined,
  tap((event) => {
    if (event.type === FSEventType.Remove) {
      removePartialFromCaches(event.file);
    }
  })
);
    ...
    tap((event) => {
      if (event.type === FSEventType.Remove) {
        if (event.file.endsWith(LAYOUT_SUFFIX)) {
          ...
          removeCacheEntry(computePageURLOf(File().setSource(event.file)));
        } else {
          // If file is not layout then it is definitely an include.
          removePartialFromCaches(event.file);
        }
      } 
    })
