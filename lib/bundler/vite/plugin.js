...
const resolveFullUrl = (url) =>
  url.endsWith(URL_DELIMITER) ? `${url}index${HTML_EXTENSION}` : url;
...
export const createPlugin = (postdocConfiguration) => {
  ...
  return {
    ...
    config: (config, _envs) => {
      if (config.server) {
        if (
          typeof config.server.open !== 'string' ||
          typeof config.server.$$openByCLI === 'string'
        ) {
      ...
      if (config.preview) {
        if (
          typeof config.preview.open !== 'string' ||
          typeof config.preview.$$openByCLI === 'string'
        ) {
    ...
    configureServer:
      ({ws, middlewares: app, ssrFixStacktrace, transformIndexHtml}) =>
        () => {
              ...
              try {
                ...
                if (!page) {
                  ...
                  response.end(
                    createErrorOverlay(
                      404,
                      `The <b>${currentPageUrl}</b> page is not found.`
                    )
                  );
                ...
                const transformedPageDescriptor = [
                  page.url,
                  page.output.source()
                ];
