...
const {MessageBuilder, error: logError, LineBuilder} = require('../logger/index.js');
 ...
 */
const createRelativeResolve = (to, includesDirectory, page) => (includePath) => {
  ...
  if (normalizedIncludePath.startsWith('~')) {
    ...
    return normalizedIncludePath.replace('~', levelsUp);
  }

  const absolutePath = path.resolve(
    normalizedIncludePath.startsWith('.') ? to.source() : includesDirectory,
  ...
  switch (true) {
    ...
    default: {
      MessageBuilder()
        .line(
          LineBuilder()
            .text('There is no file with path')
            .phrase(Typography().bold(`${absolutePath}(.ejs)`))
            .text('.')
            .build()
        )
...
const createEjsRequireResolve = (file) => (modulePath) => {
  ...
  return modulePath.startsWith('.')
    ? path.resolve(file.source(), normalizedModulePath)
...
const createGlobalAPI = ({
  ...
  const resolveRelative = createRelativeResolve(file, includesDirectory, page);
  ...
  return {
     ...
     * @returns {string}
     */
    url: (pathToFile) => resolveRelative(pathToFile).replace(process.cwd(), '~'),
    /**
    ...
    include: async (includePath, data = {}) => {
      ...
      if (page._cache.has(includeAbsolutePath)) {
        return page._cache.get(includeAbsolutePath);
      }
      ...
      return includeFile
        ...
        .chain((content) =>
          compileEjs(
            content,
            createGlobalAPI({file: includeFile, page, pages, parentData: wholeData}),
            {filename: file.source()}
          )
        )
        .map((html) => {
          page._cache.set(includeAbsolutePath, html);

          return html;
        })
        .run()
        // Any errors was cought by `compileEjs` function so we can safely extract
        // the value here.
     ...
     *
     * @returns {PublicLimitedPageInfo|PublicExtendedPageInfo}
     */
    get page() {
    ...
    /**
     * Returns a list of all standalone project pages.
     *
