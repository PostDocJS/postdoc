...
export const createGlobalAPI = ({
  ...
  return {
    ...
    url: (pathToFile) => {
      ...
      const sourceAbsolutePath = postDocResolve(pathToFile);

      return relative(dirname(outputPath), sourceAbsolutePath)
        .split(sep)
        .join(URL_DELIMITER);
    },
    /**
    ...
    include: async (includePath, data = {}) => {
            ...
            MessageBuilder()
              .line(
                LineBuilder()
                  ...
                  .phrase(
                    Typography().bold(
                      `${includeAbsolutePath.replace(cwd(), '~')}.(ejs|md)`
                    )
                  )
                  .text('path.')
                  .build()
              ...
              .line(
                'Consider providing an extension or check if the path is valid.'
              )
      ...
      const includeKeyDescriptor = descriptor.concat({
        file: includeAbsolutePath,
        data
      });
      ...
      if (includeFile.extension() === MD_EXTENSION) {
        html = await compileMarkdown(html, {
          filename: includeFile.source(),
    ...
    get page() {
      const language =
        frontMatter.language || page.language || i18n.defaultLanguage;

      const base = {...frontMatter, language, url: page.url};

      return page.layout === file ? {...base, content: layoutContent} : base;
    },
    /**
