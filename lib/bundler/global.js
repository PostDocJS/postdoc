...
import {EJS_SUFFIX, MD_SUFFIX, URL_DELIMITER} from '../constants.js';
 ...
 * @returns {string | URL}
 */
const convertPathToFileUrlIfNeeded = (path) => path.startsWith('.') || path.startsWith(sep) ? pathToFileURL(path) : path;
...
export const createGlobalAPI = ({
  ...
  const {i18n, directories: {includes}} = Container.get(CONFIGURATION_ID);
  ...
  return {
    ...
    include: async (includePath, data = {}) => {
      ...
      if (extname(includeAbsolutePath) === '') {
        switch (true) {
          case File(includeAbsolutePath + EJS_SUFFIX).exists():
            includeAbsolutePath += EJS_SUFFIX;
            break;
          case File(includeAbsolutePath + MD_SUFFIX).exists():
            includeAbsolutePath += MD_SUFFIX;
            break;
          default: {
            MessageBuilder()
              .line(
      ...
      if (includeFile.extension() === MD_SUFFIX) {
