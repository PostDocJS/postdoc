...
const {sep, resolve, dirname, extname, normalize, isAbsolute} = require('path');
...
const {compileEjs} = require('./page/compile-ejs.js');
...
/**
 * Creates a `resolve` function for the custom _include_ pseudo global function.
 *
...
const createRelativeResolve = (to, includesDirectory) => (includePath) => {
  ...
  const absolutePath = normalizedIncludePath.startsWith('~')
    ? normalizedIncludePath.replace('~', cwd())
    : resolve(
      includePath.startsWith('.') ? dirname(to.source()) : includesDirectory,
      normalizedIncludePath
    );

  if (extname(normalizedIncludePath) === '') {
    switch (true) {
      case File(absolutePath + '.ejs').exists(): return absolutePath + '.ejs'; 
      default: {
        MessageBuilder()
          .line(
            LineBuilder()
              .text('Cannot find a file following the')
              .phrase(Typography().bold(`${absolutePath.replace(cwd(), '~')}(.ejs)`))
              .text('path.')
              .build()
          )
...
/**
 * Creates a `resolve` function for the JavaScript code embedded into
 * EJS templates.
 *
 * @param {import('../files.js').IFile} file
 * @returns {(modulePath: string) => string}
 */
const createRequireResolveShim = (file) => (modulePath) => {
  const normalizedModulePath = normalize(modulePath);

  return modulePath.startsWith('.')
    ? resolve(dirname(file.source()), normalizedModulePath)
    : builtinModules.includes(normalizedModulePath) || isAbsolute(normalizedModulePath) 
      ? normalizedModulePath
      : resolve('node_modules', normalizedModulePath);
};

/**
 * Creates the `require` function to be used in ESJ templates.
 *
 * @param {import('../files.js').IFile} file
 */
const createRequireShim = (file) => {
  const requireResolve = createRequireResolveShim(file);

  return Object.assign(pipe(requireResolve, require), {resolve: requireResolve});
}; 
...
const createGlobalAPI = ({
  ...
  const resolveRelative = createRelativeResolve(file, includes);
  ...
  return {
     ...
     * @returns {unknown}
     */
    require: createRequireShim(file),
    /**
