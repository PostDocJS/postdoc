 ...
 */
export const compileMarkdown = async (content, {filename, withFrontMatter = false}) => {
  const [frontMatter, body] = splitFrontMatterAndContent(content);

  try {
    return [
      withFrontMatter && frontMatter ? parseFrontMatter(frontMatter, filename) : null,
      marked.parse(body)
    ];
  } catch (err) {
    MessageBuilder()
