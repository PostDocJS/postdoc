 ...
 * @returns {boolean}
 */
const isLayout = (file) => file.name().endsWith(LAYOUT_EXTENSION);

const NON_PAGE_NAMES = ['readme', 'license'];
...
export const isPage = (file) =>
  !isPartial(file) &&
  !isLayout(file) &&
  NON_PAGE_NAMES.every((name) => !file.name().toLowerCase().includes(name)) &&
...
const findLayoutIn = (directory, basename, stopAt) => {
  ...
  if (directory.exists()) {
    ...
    layout =
      files.find((file) => file.name() === `${basename}${LAYOUT_EXTENSION}`) ||
      files.find((file) => file.name() === `index${LAYOUT_EXTENSION}`);
  }

  return (
    layout ||
    (directory.source() === stopAt.source()
      ? File()
...
const createPageBuilder =
   ...
   */
    (contentFile) => {
      ...
      const language = configuration.i18n.languages.find((language) =>
        outputFilePath.includes(`${sep}${language}`)
      );
...
export const getAllPages = (configuration) => {
  ...
  const isMatch = picomatch(configuration.ignore.pages);

  const pages = Directory(configuration.directories.pages)
    .recursive(true)
    .files()
    .filter(isPage)
    .filter(pipe((file) => file.source(), not(isMatch)))
    ...
    MessageBuilder()
      ...
      .line(
        LineBuilder()
          ...
          .phrase(
            Typography().bold(
              configuration.directories.layouts.replace(cwd(), '~')
            )
          )
          .phrase('directory.')
          .build()
