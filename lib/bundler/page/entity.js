 ...
 * @returns {boolean}
 */
export const isPageFile = (file) => !isPartialFile(file) && (file.extension() === MD_SUFFIX || file.extension() === EJS_SUFFIX);
...
const findLayoutIn = (directory, basename, stopAt) => {
  ...
  if (directory.exists()) {
    ...
    layout = files.find((file) => file.name() === `${basename}${EJS_SUFFIX}`)
      || files.find((file) => file.name() === `index${EJS_SUFFIX}`);
  }
...
const createPageBuilder = (configuration) =>
   ...
   */
    ...
    let outputFilePath = contentFile
      ...
      .replace(EJS_SUFFIX, HTML_SUFFIX);
    ...
    return {
      url,
      isMD: contentFile.extension() === MD_SUFFIX,
      language,
      output: outputFile,
      layout: layoutFile,
      content: contentFile
