...
const {not, map, filter, compose} = require('../utils/fp.js');
const {
  MD_SUFFIX,
  JS_SUFFIX,
  TS_SUFFIX,
...
const PublicApiOf = (pages) =>
  ...
  (currentPage) => ({
    /**
     * Returns an array of parsed API entries for each member of tracked files.
     *
     * @param {...(value: unknown) => unknown} maps - a list of map function that transforms
     *   a value produced by the previous map function of the StructuredAPIFile in case of
     *   the first map. If a `map` returns _undefined_ then that value is omitted from
     *   the final array.
    ...
    api: async (...maps) =>
      getAPIData()
        .then((entities) =>
          entities.reduce(
            compose(...maps.flatMap((fn) => [map(fn), filter((value) => value !== undefined)]))(
              (accumulator, value) => accumulator.concat([value])
            ),
            []
          )),
    /**
     * Rebase the file path to be picked up by Vite.
     * It has to start with the *~* character.
     *
     * @param {string} filePath
     */
    url: (filePath) => {
      const temporalOutputPath = currentPage.temporaryOutput.source();
      ...
      return filePath.replace('~', levelsUp);
    },
    /**
