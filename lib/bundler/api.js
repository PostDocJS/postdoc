    ...
    comments
      ...
      .map(comment =>
        comment.tags.reduce((result, tag) => {
          ...
          result[API_TEMPLATE.NAME] = nameTag?.string || comment.ctx?.name || basename(path);

          result[API_TEMPLATE.KIND] =
            [DOC_TAGS.METHOD, DOC_TAGS.FUNCTION, DOC_TAGS.FUNC].includes(kindTag?.type)
              ? DOC_TAGS.FUNCTION
          ...
          switch (tag.type) {
            ...
            case DOC_TAGS.PARAM: {
              ...
              return {
                ...result,
                [API_TEMPLATE.PARAMETERS]: result[API_TEMPLATE.PARAMETERS].concat([
                  {
            ...
            case DOC_TAGS.ALIAS: {
              return {
                ...result,
                [API_TEMPLATE.ALIASES]: result[API_TEMPLATE.ALIASES].concat([tag.string])
              };
            }
            ...
            case DOC_TAGS.SYNTAX: {
              return {
                ...result,
                [API_TEMPLATE.SYNTAX]: result[API_TEMPLATE.SYNTAX].concat([tag.string])
              };
            }
            case DOC_TAGS.SEE: {
              return {
                ...result,
                [API_TEMPLATE.SEE]: result[API_TEMPLATE.SEE].concat([tag.string])
              };
            }
