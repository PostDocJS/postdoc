 ...
 *
 * > We use `npx` in examples above to use the `postdoc` binary directly
      ...
      runAndMeasureAction(async () => {
        ...
        if (name === ".") {
          ...
          if (await AsyncIterable.isEmpty(directoryDescriptor.files)) {
            ...
            Logger.log(
              (typography) => `
    						The ${typography.bold(targetDirectoryName)} directory is not empty.
    						Please emtpy the folder and try again.
     					`,
            );
        ...
        const nightwatchVersion = await getPackageVersion("nightwatch").catch(
          () => "3.0.0",
        );
