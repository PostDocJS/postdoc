...
import {sep, join, normalize, extname} from 'node:path';
...
export const test = () =>
  new Command('test')
    ...
    .action(async (pagePath, options) => {
      ...
      const prefixPageName = uriParts.slice(0, uriParts.length - 1).join(URL_DELIMITER);
      ...
      const isDirectories = [testsDirectories, options.output].map(testDir => extname(testDir) === '');
        ...
        return isDirectories.forEach((isDir, index) => {
          if (!isDir) {
            MessageBuilder()
              ...
              .line(
                LineBuilder()
                  .text('Please, write the path with the directory and repeat again.')
                  .build()
              )
      ...
      const outputTest = options.output
        ? testsDirectories.find(testDirectories => normalize(testDirectories).split(sep).pop() === options.output)
        : prefixPageName;
        ...
        return MessageBuilder()
          .line(
            LineBuilder()
              ...
              .phrase('The test dir')
              .phrase(DEFAULTS.directories.tests + URL_DELIMITER + options.output)
        ...
        .map(({source, destination: [base]}) => {
          ...
          if (!destination) {
            MessageBuilder()
              .line(
                LineBuilder()
                  .text('It is impossible to determine where the test file should be written.')
                  .build()
              )
              .line('Add the option -o | --output and specify the path where to write the test file.')
              .map(Typography().red)
              .pipe(logError);
