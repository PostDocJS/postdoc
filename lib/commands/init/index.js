...
export const init = () =>
  new CustomCommand('init')
    ...
    .action(async (options) => {
      ...
      await File(resolveAsset('files', 'layout.ejs'))
        .write(join(pagesDirectory.source(), 'index' + LAYOUT_EXTENSION));

      const pages = [
        {name: 'homepage', path: join(pagesDirectory.source(), 'index.ejs')},
        {name: 'api_reference', path: join(pagesDirectory.source(), 'api_reference', 'index.ejs')},
        {name: 'about', path: join(pagesDirectory.source(), 'about', 'index.ejs')},
        {name: 'installation', path: join(pagesDirectory.source(), 'guide', 'installation',  'index.ejs')},
        {name: 'usage', path: join(pagesDirectory.source(), 'guide', 'usage',  'index.ejs')},
        {name: 'configuration', path: join(pagesDirectory.source(), 'api_reference', 'configuration',  'index.ejs')},
        {name: 'docs', path: join(pagesDirectory.source(), 'api_reference', 'docs',  'index.ejs')},
        {name: 'concepts', path: join(pagesDirectory.source(), 'guide', 'concepts',  'index.ejs')},
        {name: 'what_is_project', path: join(pagesDirectory.source(), 'guide', 'index.ejs')}
      ];

      const components = [
        {name: 'header', path: join(includesDirectory.source(), 'header.ejs')},
        {name: 'left_side_menu', path: join(includesDirectory.source(), 'left_side_menu.ejs')},
        {name: 'left_api_menu', path: join(includesDirectory.source(), 'left_api_menu.ejs')},
        {name: 'mobile_sidesmenu_header', path: join(includesDirectory.source(), 'mobile_sidesmenu_header.ejs')},
        {name: 'footer', path: join(includesDirectory.source(), 'footer.ejs')}
      ];

      const assets = [
        {name: 'background.png', path: join(workingDirectory, 'assets', 'homepage_background.jpg')},
        {name: 'project_image.png', path: join(workingDirectory, 'assets', 'project_image.jpg')},
        {name: 'code_icon.svg', path: join(workingDirectory, 'assets', 'code_icon.svg')},
        {name: 'facebook_light.svg', path: join(workingDirectory, 'assets', 'facebook_light.svg')},
        {name: 'facebook.svg', path: join(workingDirectory, 'assets', 'facebook.svg')},
        {name: 'github_light.svg', path: join(workingDirectory, 'assets', 'github_light.svg')},
        {name: 'github.svg', path: join(workingDirectory, 'assets', 'github.svg')},
        {name: 'instagram_light.svg', path: join(workingDirectory, 'assets', 'instagram_light.svg')},
        {name: 'instagram.svg', path: join(workingDirectory, 'assets', 'instagram.svg')},
        {name: 'light_icon.svg', path: join(workingDirectory, 'assets', 'light_icon.svg')},
        {name: 'lightbulb_icon.svg', path: join(workingDirectory, 'assets', 'lightbulb_icon.svg')},
        {name: 'logo_light.svg', path: join(workingDirectory, 'assets', 'logo_light.svg')},
        {name: 'logo.svg', path: join(workingDirectory, 'assets', 'logo.svg')},
        {name: 'moon.svg', path: join(workingDirectory, 'assets', 'moon.svg')},
        {name: 'search_icon_light.svg', path: join(workingDirectory, 'assets', 'search_icon_light.svg')},
        {name: 'search_icon.svg', path: join(workingDirectory, 'assets', 'search_icon.svg')},
        {name: 'sun.svg', path: join(workingDirectory, 'assets', 'sun.svg')},
        {name: 'twitter_light.svg', path: join(workingDirectory, 'assets', 'twitter_light.svg')},
        {name: 'twitter.svg', path: join(workingDirectory, 'assets', 'twitter.svg')}
      ];
      ...
      for (const cssFile of cssFiles) {
        ...
        await File(resolveAsset('styles', fileName))
          .write(join(workingDirectory, 'css', fileName));
      }

      for (const jsFile of jsFiles) {
        const fileName = `${jsFile}.js`;

        await File(resolveAsset('scripts', fileName))
          .write(join(workingDirectory, 'js', fileName));
      }
      ...
      await File(resolveAsset('files', 'robots.txt')).write(join(
