...
import {AssetBuilder, AssetKind, resolveAsset} from '../../assets/manager.js';
  ...
  new CustomCommand('init')
    ...
    .action(async (options) => {
      ...
      const directoriesToCreate = [
        ...
        join(workingDirectory, 'assets'),
        join(workingDirectory, 'css'),
        join(workingDirectory, 'js')
      ];
      ...
      const pages = [
        {name: 'homepage', path: join(workingDirectory, 'pages', 'index.ejs')},
        ...
        {name: 'installation', path: join(workingDirectory,  'pages', 'guide', 'installation',  'index.ejs')},
        {name: 'usage', path: join(workingDirectory,  'pages', 'guide', 'usage',  'index.ejs')},
        {name: 'configuration', path: join(workingDirectory,  'pages', 'api_reference', 'configuration',  'index.ejs')},
        {name: 'docs', path: join(workingDirectory,  'pages', 'api_reference', 'docs',  'index.ejs')},
        {name: 'concepts', path: join(workingDirectory,  'pages', 'guide', 'concepts',  'index.ejs')},
        {name: 'what_is_project', path: join(workingDirectory, 'pages', 'guide', 'index.ejs')}
      ];
      ...
      for (const component of components) {
        await createComponent(component.name, component.path)();
      }
      for (const page of pages) {
        await createPage(page.name, page.path)();
      }
      for (const cssFile of cssFiles) {
        const outputPath = join(workingDirectory, 'css', `${cssFile}.css`);
      ...
      for (const jsFile of jsFiles) {
        ...
        await File(resolveAsset('files', fileName))
          .write(join(workingDirectory, 'js', fileName));
      }

      for (const asset of assets) {
        const outputPath = join(workingDirectory, 'assets', `${asset.name}.jpg`);
