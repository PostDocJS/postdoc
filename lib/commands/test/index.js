...
const {MessageBuilder, StatusLine, info, LineBuilder, Separator, warn} = require('../../logger/index.js');
...
exports.test = () =>
  new Command('test')
    ...
    .action(async () => {
      ...
      const doesTestsDirectoryExist = await Directory()
        .setSource(path.resolve(directories.tests))
        .exists();
      ...
      spawn('npx', ['nightwatch', ...process.argv], {
        stdio: 'inherit'
      })
        .on('close', () => 
          MessageBuilder()
