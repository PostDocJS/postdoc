...
const {Ok, Err, mergeResults} = require('./result.js');
 ...
 * @template [F=never]
 * @typedef {T extends readonly [IFuture<infer A, infer B>, ...infer Rest] ? MergedFutures<Rest, [...S, A], F | B> : T extends readonly [IFuture<infer I, infer L>] ? [[...S, I], F | L] : T extends readonly [] ? [S, F] : T extends readonly IFuture<infer W, infer M>[] ? [W, M] : never} MergedFutures
 */
 ...
 * @returns {IFuture<MergedFutures<Futures>[0], MergedFutures<Futures>[1]>}
 */
const mergeFutures = (futures) =>
  Future((done, fail) =>
    ...
    .map(mergeResults)
    .chain((result) => result.map(Succeed).extract(Fail));
