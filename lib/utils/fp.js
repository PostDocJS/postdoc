 ...
 */
const parallel = (...fns) => (value) => fns.map((fn) => fn(value));
 ...
 */
const bimap = (predicate, mapOnTrue, mapOnFalse) =>
  (reducer) =>
    (accumulator, value) =>
 ...
 * @returns {Array<T>}
 */
const concat = (accumulator, value) => accumulator.concat(value);
 ...
 */
const when = (predicate, onTrue, onFalse = () => null) => (...args) =>
 ...
 */
const memo = (fn, keyFrom = JSON.stringify) => {
  const cache = new Map();
