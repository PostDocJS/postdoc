...
describe("Configuration module", function () {
  ...
  describe("files priority", function () {
    it("should load the ES module config file at first", async function (client) {
      await fs.writeFile(
        path.join(tmpDir, "postdoc.conf.mjs"),
        "export default {server: {base: '/root'}}",
      );
      await fs.writeFile(
        path.join(tmpDir, "postdoc.conf.cjs"),
        "module.exports = {server: {port: 5698}}",
      );
      await fs.writeFile(
        path.join(tmpDir, "postdoc.conf.js"),
        "module.exports = {server: {strictPort: true}}",
      );
      await fs.writeFile(
        path.join(tmpDir, "postdoc.json"),
        '{"server": {"force: true}}',
      );
      await fs.writeFile(
    ...
    it("should load the config as ES module if package.json contains the type: module", async function (client) {
      await fs.writeFile(
        path.join(tmpDir, "postdoc.conf.js"),
        'export default {server: {strictPort: true}, prop: "${PROP_ENV}"}',
      );
      await fs.writeFile(
        path.join(tmpDir, "postdoc.json"),
        '{"server": {"force": true}}',
  ...
  describe("Injecting environment variables", function () {
    it("should replace the interpolation pattern with an environment variable with the pattern as the name", async function (client) {
      await fs.writeFile(
        ...
        'PROP_ENV = "cool secret value"',
      );
      await fs.writeFile(
        path.join(tmpDir, "postdoc.conf.js"),
        'export default {prop: "${PROP_ENV}"}',
      );
      await fs.writeFile(
        path.join(tmpDir, "package.json"),
        '{"type":"module"}',
